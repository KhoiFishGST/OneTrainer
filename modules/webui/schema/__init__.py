...
def build_training_tab(model_type: ModelType) -> Tab:
    groups = []
...
class SchemaRegistry:
    ...
    def get_all_field_names(cls) -> list[str]:
        ...
        for mt in ModelType:
            for tab in (build_model_tab(mt), build_training_tab(mt)):
                for group in tab.groups:
                    for field in group.fields:
                        field_names.extend([k.split(".")[0] for k in field.keys])
    ...
    def build(self, model_type: str, training_method: str) -> dict[str, object]:
        ...
        for tab in TABS:
            ...
            elif tab.id == "training":
                dynamic_tab = build_training_tab(model_type_enum)
                dynamic_tabs.append(dynamic_tab.to_dict(config_template))
            else:
