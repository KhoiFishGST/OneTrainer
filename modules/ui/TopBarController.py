...
from modules.util.config import config_io
...
class TopBarController:
    ...
    def load_preset_tree(self, dir: str = "training_presets") -> list[tuple[str, str | list]]:
        return config_io.load_preset_tree(dir, include_user_files=False)

    def save_to_file(self, name) -> str:
        return str(config_io.save_named_preset(self.train_config, name, "training_presets"))

    def save_config_to_path(self, path: str) -> None:
        config_io.save_settings(self.train_config, path)

    def load_config_from_file(self, filename: str) -> TrainConfig | None:
        try:
            loaded_config = config_io.load_train_config(filename, secrets_path="secrets.json")
    ...
    def save_secrets(self, path) -> str:
        return str(config_io.save_secrets(self.train_config, path))
